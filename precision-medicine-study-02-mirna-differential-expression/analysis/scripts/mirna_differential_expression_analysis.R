...
setwd("C:/Users/mmsid/OneDrive/Documents/1. Mahima Mahabaleshwar_IUPUI Documents/1. IUPUI Semester Folder/3rd Semester IUI Fall 2024/2. Precesion Medicine/Assignment/Assignment-2/New File")
...
if (!file.exists("miRNA_counts.txt")) stop("File 'miRNA_counts.txt' not found.")
counts <- read.table("miRNA_counts.txt", header = TRUE, row.names = 1)

# Select columns with SRR identifiers (count data) and clean the column names
counts_filtered <- counts[, grepl("^X\\.N\\.slate\\.msiddhe\\.Assignment\\.2\\.hisat2_aligned_bam\\.SRR", colnames(counts))]
colnames(counts_filtered) <- sub("_aligned_sorted\\.bam$", "", colnames(counts_filtered))
...
# Save CSV File
# Save DEG results for the first comparison (24H vs 72H) as a CSV file
write.csv(deg_24h_vs_72h, file = "DEG_Comparison_24H_vs_72H.csv", row.names = TRUE)
# Save DEG results for the second comparison (Control vs Infected) as a CSV file
write.csv(deg_control_vs_infected, file = "DEG_Comparison_Control_vs_Infected.csv", row.names = TRUE)
